...
def attempt_to_fix_json_by_finding_outermost_brackets(json_string):
    ...
    logger.typewriter_log("JSON数据格式似乎出了一些问题，我正在尝试修复它\n")
    ...
    try:
        ...
        if json_match:
            ...
            logger.typewriter_log(title="JSON数据格式修复完成。", title_color=Fore.GREEN)
            if cfg.speak_mode and cfg.debug_mode:
               speak.say_text("JSON数据格式修复完成。")
        else:
...
def print_assistant_thoughts(assistant_reply):
    ...
    try:
        ...
        if cfg.translate_api_name and (cfg.translate_api_name.strip() if cfg.translate_api_name else None):
            has_translated = True
            if cfg.translate_api_name == "baidu":
                assistant_thoughts_text = translate(assistant_thoughts_text, "en", "zh")
                assistant_thoughts_reasoning = translate(assistant_thoughts_reasoning, "en", "zh")
                assistant_thoughts_criticism = translate(assistant_thoughts_criticism, "en", "zh")

        logger.typewriter_log(f"{ai_name.upper()} 我的想法是:", Fore.YELLOW, assistant_thoughts_text)
        logger.typewriter_log("我是这么推理的:", Fore.YELLOW, assistant_thoughts_reasoning)

        if assistant_thoughts_plan:
            logger.typewriter_log("我计划要做的事:", Fore.YELLOW, "")
            ...
            for line in lines:
                line = line.lstrip("- ")
                if has_translated:
                    logger.typewriter_log("- ", Fore.GREEN, translate(line.strip(), "en", "zh"))
                else:
                    logger.typewriter_log("- ", Fore.GREEN, line.strip())

        logger.typewriter_log("我对此的评论:", Fore.YELLOW, assistant_thoughts_criticism)
...
def construct_prompt():
    ...
    if cfg.skip_reprompt and config.ai_name:
        ...
        logger.typewriter_log("目标:", Fore.GREEN, config.ai_goals)
    elif config.ai_name:
        logger.typewriter_log(
            f"欢迎回来",
            Fore.GREEN,
            f"想要让我变回 {config.ai_name}?",
            speak_text=True)
        should_continue = utils.clean_input(f"""继续执行上一次未完的任务么?
Name:  {config.ai_name}
Role:  {config.ai_role}
...
def prompt_user():
    ...
    logger.typewriter_log(
        "Welcome to Auto-GPT! ",
        Fore.GREEN,
        "在下面输入你的AI的名字和它的角色。什么都不输入将加载默认值。",
        speak_text=True)
    ...
    logger.typewriter_log(
        ...
        Fore.GREEN,
        "比如, '不用付工资的员工 A107159'")
    ai_name = utils.clean_input("请输入AI的名字: ")
    if ai_name == "":
        ai_name = "员工 A107159"
    ...
    logger.typewriter_log(
        "对你的AI做一段角色描述: ",
        Fore.GREEN,
        "比如, '这是一款旨在自主开发和运营业务的人工智能，唯一目标是增加你的净资产。'")
    ai_role = utils.clean_input(f"{ai_name} 的描述是: ")
    if ai_role == "":
        # ai_role = "an AI designed to autonomously develop and run businesses with the sole goal of increasing your net worth."
        ai_role="这是一款旨在自主开发和运营业务的人工智能，唯一目标是增加你的净资产。"

    # Enter up to 5 goals for the AI
    logger.typewriter_log(
        "为你的AI输入最多5个目标: ",
        Fore.GREEN,
        "例如:\n增加资产，增加推特账户，自主开发和管理多项业务")
    print("不输入任何内容以加载默认值，完成时不输入任何内容。", flush=True)
    ai_goals = []
    for i in range(5):
        ai_goal = utils.clean_input(f"{Fore.LIGHTBLUE_EX}任务{Style.RESET_ALL} {i+1}: ")
        if ai_goal == "":
            break
...
def parse_arguments():
    ...
    if args.speak:
        logger.typewriter_log("Speak Mode(语音模式): ", Fore.GREEN, "启用")
        cfg.set_speak_mode(True)

    if args.gpt3only:
        logger.typewriter_log("GPT3.5 Only Mode(仅使用gpt3.5接口): ", Fore.GREEN, "启用")
        cfg.set_smart_llm_model(cfg.fast_llm_model)

    if args.gpt4only:
        logger.typewriter_log("GPT4 Only Mode(仅使用GPT4模式): ", Fore.GREEN, "启用")
        ...
        while True:
            ...
            if not cfg.continuous_mode and self.next_action_count == 0:
                ...
                logger.typewriter_log(
                    "下一步要执行: ",
                    Fore.CYAN,
                    f"要执行的命令是 = {Fore.CYAN}{command_name}{Style.RESET_ALL}  执行的参数是 = {Fore.CYAN}{arguments}{Style.RESET_ALL}")
                print(
                    f"输入命令 'y' 授权执行, 数据命令'y -N' 授权并连续运行N个命令, 输入命令'n' 退出程序, 或者可以输入反馈给 {self.ai_name}...",
                    flush=True)
                while True:
                    console_input = utils.clean_input(Fore.MAGENTA + "请输入命令:" + Style.RESET_ALL)
            ...
            if result is not None:
                ...
                if has_translated:
                    result = translate(result,"en","zh")
                logger.typewriter_log("命令输出: ", Fore.YELLOW, result)
            else:
